...
{
	read_file_list("file_list_MRT.txt");
	//read_file_list("file_list_rain.txt");
	...
	strcpy(file_csv_data,"../../../../csv_data/BigData_20150103_2048_new.csv");
	read_raw_data(); 
	clock_t end1 = clock();
	...
	for(int i=0;i<month_vec.size();i++)
	{
		for(int j=0;j<month_vec[i].day_vec.size();j++)
		{
			for(int u=0;u<dim;u++)
			{
				...
				model.at<float>(t,u) = month_vec[i].day_vec[j].data[u]/100000.0;
				//model.at<float>(t,u) = month_vec[i].day_vec[j].data[u]/100.0;	
			}
			t++;
		}
	}
	...
	*/
	...
	rearrange_mat_by_sort_color_index(lab_color_sort_index,cluster_centers,cluster_tag,rgb_mat2);
	clock_t end4 = clock();
	...
	*/
	...
	Mat raw_data_lab;
	Mat raw_data_3D = lab_alignment_new_convexhull(model).clone();
	clock_t end10 = clock();
...
void Preprocessing_Data::read_file_list(char* file_name)
{
	FILE* file;
...
void Preprocessing_Data::read_single_file(char* file_name)
{
	FILE* file = NULL;
	file = fopen(file_name,"r");
	...
	char str[5][80];
	int y,m,d;
	int red,orange,total;
	char week[80];
	fscanf(file, "%s,%s,%s,%s,%s\n", str[0],str[1],str[2],str[3],str[4]);
	while (!feof(file))
	{
		fscanf(file, "%d/%d/%d,%s,%d,%d,%d\n", &y,&m,&d,week,&red,&orange,&total);
		cout << y << "/" << m << "/" << d << " " << red << " " << orange << " " << total << endl;
	}
	*/
...
Mat Preprocessing_Data::Gaussian_filter(int* attribute_title)
{
	...
	//Apply Gaussian filter to raw data
	for(int i=0;i<raw_data.size();i++)
	{
		for(int j=0;j<9;j++)
		{
...
void Preprocessing_Data::interpolate_distance(Mat& first_order_distance_mat,int interval)
{
	for(int i=0;i<raw_data.size()-1;i++)
	{
		...
		{
			//cout << "i " << i << " current index " << cur_index << endl;
			cur_index++;
			while( first_order_distance_mat.at<float>(0,cur_index) == 0.0)
			{
				...
				if(count>interval || cur_index>raw_data.size()-1) break;
			}
...
void Preprocessing_Data::voting_for_data(int k,Mat cluster_tag)
{
...
Mat Preprocessing_Data::lab_alignment_new_convexhull(Mat cluster_center)
{
	int vTotal = lab_vertices.size();
	//Turn LAB vector into LAB mat
	Mat lab_mat = Mat::zeros(vTotal,3,CV_32F);
	...
	/*
	//Mat R = Mat(100, 3, CV_8UC3);
    //randu(R, Scalar::all(0), Scalar::all(255));
	vector<Point2i> points;
	for(int i=0;i<lab_alignment_mat.rows;i++)
	{
		Point2i pt;
		//pt.x = R.at<unsigned char>(i,0);
		//pt.y = R.at<unsigned char>(i,1);		
		pt.x = lab_alignment_mat.at<float>(i,0);
		pt.y = lab_alignment_mat.at<float>(i,1);
		...
		for(int v=0;v<move_vector.size();v++)
		{
			for(int w=0;w<move_vector.size();w++)
			{
				...
				{			
						...
						{
							Mat component, coeff;
							reduceDimPCA(align_mat, 2, component, coeff);
							Mat align_mat_2D = coeff * component;

							vector<Point2i> points;
							for(int i=0;i<align_mat_2D.rows;i++)
							{
								Point2i pt;
								pt.x = align_mat_2D.at<float>(i,0);
								pt.y = align_mat_2D.at<float>(i,1);
							...
							{
								int index = hull[i];
								if(lab_boundary_test(align_mat.at<float>(index,0),align_mat.at<float>(index,1),align_mat.at<float>(index,2))==false)
								{
									flag = false;
									break;
								}
...
Mat Preprocessing_Data::lab_alignment_binarySearch(Mat cluster_center, Mat& lab_raw)
{
	int vTotal = lab_vertices.size();
	//Turn LAB vector into LAB mat
	Mat lab_mat = Mat::zeros(vTotal,3,CV_32F);
			...
			for(int w=0;w<move_vector.size();w++)
			{
				...
				{			
						...
						while(low <= high)
						{
							...
							cluster_center_coeff = cluster_center_coeff_const.mul(mid);
...
double Preprocessing_Data::TSP_boost_path_by_EdgeWeight(Mat input_mat, Mat& sort_index)
{
	...
	//cout << "sort index " << sort_index << endl;
...
void Preprocessing_Data::TSP_boost_for_histogram_coarse_to_fine3(Mat cluster_center, Mat& histo_sort_index)
{
 ...
 //	for(int i=0;i<group_num;i++)
	...
	//	{
	//		Ev.at<float>(i,j) = Ev.at<float>(i,j)*Ev.at<float>(i,j);
	//	}
	//}
	reduceDimPCA(Ev, rDim, component, coeff);
	Mat Ev_PCA2D = coeff * component;
	...
	for(int i=0;i<group_num;i++)
	{
		Mat Ev_sub = Mat::zeros(groups[i].index.size(),2,CV_32F);
		Mat histo_sort_index_sub = Mat::zeros(groups[i].index.size(),1,CV_32S);
		for(int j=0;j<groups[i].index.size();j++)
		{
			int index = groups[i].index[j];
